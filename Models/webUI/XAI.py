...
class ClusterXAI(DefaultConfig):
    ...
    def create_instruction(self, prompt_file):
        """
        Carrega as instruções de um arquivo de prompt.
        """
...
def main(prompt_file='Fase 1 - Direto.txt', cluster_type="K-means"):
    ...
    for alias, path in cluster_paths:
        ...
        load_file = LoadFile()
        response = load_file.upload_file_to_webui(path)
        ...
        if response:
            ...
            for temperature in temperatures:
                print(f"Executando: {alias} com temperatura {temperature}")

                ia_gen_model = IAGenModel(temperature=temperature)
                api_response = ia_gen_model.create_model(prompt)

                model.add_item_to_dataframe(results, [cluster_type, alias, temperature, prompt_file, api_response["choices"][0]["message"]["content"]])
